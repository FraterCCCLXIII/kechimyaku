...
export default async function EditIndexArticlePage({ params }: PageProps) {
  const session = await getServerSession(authOptions);
  const { id } = await params;
  const masterId = Number.parseInt(id, 10);

  if (!session?.user?.id) {
    redirect(`/login?callbackUrl=/index/${id}/edit`);
  }
  ...
  return (
    <div className="mx-auto w-full max-w-4xl space-y-4 px-4 py-6">
      <h1 className="text-2xl font-medium tracking-tight">Edit Article</h1>
